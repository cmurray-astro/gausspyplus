...
from .utils.spectral_cube_functions import (
    change_header,
    save_fits,
    correct_header,
    update_header,
...
class GaussPyDecompose(object):
    ...
    def check_settings(self):
        ...
        if self.dirpath_gpy is None:
            self.dirpath_gpy = os.path.normpath(self.dirname + os.sep + os.pardir)
    ...
    def make_cube(self, mode="full_decomposition"):
        ...
        header = update_header(self.header.copy(), comments=comments, write_meta=True)
    ...
    def create_input_table(self, ncomps_max=None):
        ...
        for idx in tqdm(range(length)):
            ...
            if self.main_beam_efficiency is not None:
                fit_amps = [amp / self.main_beam_efficiency for amp in fit_amps]
                fit_e_amps = [e_amp / self.main_beam_efficiency for e_amp in fit_e_amps]
            ...
            for j in range(ncomps):
                ...
                x_wcs, y_wcs, z_wcs = self.wcs.wcs_pix2world(xi, yi, mean_value, 0)
                ...
                velocity.append((z_wcs * self.wcs.wcs.cunit[2]).to(self.vel_unit).value)
                ...
                e_integrated_intensity = (
                    area_of_gaussian(amp_value + e_amp_value, fwhm_value + e_fwhm_value)
    ...
    def produce_component_map(self, dtype="float32"):
        ...
        pathToFile = os.path.join(os.path.dirname(self.dirname), "gpy_maps", filename)
    ...
    def produce_velocity_dispersion_map(self, mode="average", dtype="float32"):
        ...
        filename = "{}{}_{}_veldisp_map.fits".format(self.filename, self.suffix, mode)
        pathToFile = os.path.join(os.path.dirname(self.dirname), "gpy_maps", filename)

        save_fits(data.astype(dtype), header, pathToFile, verbose=False)
        say(
            ">> saved {} velocity dispersion map '{}' in {}".format(
